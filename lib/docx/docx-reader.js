...
function read(docxFile, input) {
    ...
    return promises.props({
        ...
        return {
            ...
            footer: readXmlFileWithBody(result.partPaths.footer, result, function(bodyReader, xml) {
                if (xml) {
                    var reader = new DocumentXmlReader({
                        bodyReader: bodyReader,
                        notes: new Result([]),
                    ...
                    return reader.convertXmlToDocument(xml, "w:ftr");
                } else {
                    return new Result([]);
                }
            })
        ...
        return {
            notes: result.footnotes.smartMap(function(footnotes) {
        ...
        return readXmlFileWithBody(result.partPaths.mainDocument, result, function(bodyReader, xml) {
            return result.notes.smartMap(function(notes) {
                return result.comments.smartMap(function(comments) {
                    var reader = new DocumentXmlReader({
                        bodyReader: bodyReader,
                        notes: notes,
                        comments: comments
    ...
    return readPackageRelationships(docxFile).then(function(packageRelationships) {
        ...
        return xmlFileReader({
            ...
            return {
                ...
                footer: findPartRelatedToMainDocument("footer1")
            };
        });
    });
}
