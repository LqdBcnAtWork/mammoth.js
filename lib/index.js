            ...
            return docxReader.read(docxFile, input)
                .then(function(documentResult) {
                    if (options.returnPart == "document") {
                        return documentResult.value.document.map(options.transformDocument);
                    } else if (options.returnPart == "header") {
                        return documentResult.value.header.map(options.transformDocument);
                    } else if (options.returnPart == "footer") {
                        return documentResult.value.footer.map(options.transformDocument);
                    } else {
                        return documentResult.value.document.map(options.transformDocument);
                    }
                })
...
function convertDocumentToHtml(documentResult, options) {
    ...
    return documentResult.smartMap(function(document) {
        return styleMapResult.smartMap(function(styleMap) {
...
function extractRawText(input) {
    return unzip.openZip(input)
        ...
        .then(function(documentResult) {
            return documentResult.value.document.map(convertElementToRawText);
        });
}
